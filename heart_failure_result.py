...
heart_data = pd.read_csv("HEART_MODEL/heart_failure.csv")
x = heart_data.drop(columns="HeartDisease", axis=1)

# Initialize the scaler and fit it on the original data
scaler = StandardScaler()
scaler.fit(x)

# Function to predict heart failure risk and provide only necessary information
def predict_heart_failure(input_data):
    input_data_as_numpy_array = np.asarray(input_data).reshape(1, -1)
    
    # Convert the numpy array to a DataFrame with the same feature names as the training data
    ...
    # Get the probability of the positive class (having heart failure)
    if prediction == 0:
        risk = "Your not having heart failure"
        disease_type = "No Heart Failure"
        risk_percentage = 0.0  # Set risk percentage to 0 for healthy individuals
    else:
        risk = "You are at risk of heart failure"
...
def generate_prevention_report(risk, disease_type, age):
    report = f"""
...
print("Please enter the following values for heart failure prediction:")
input_data = [
    int(input("Enter Age: ")),
    int(input("Enter Sex (1 = male, 0 = female): ")),
    int(input("Enter Chest Pain Type (0, 1, 2, 3): ")),
    int(input("Enter Resting BP (resting blood pressure in mm Hg): ")),
    int(input("Enter Cholesterol (serum cholesterol in mg/dl): ")),
    int(input("Enter Fasting Blood Sugar (1 = true, 0 = false): ")),
    int(input("Enter Resting ECG (0, 1, 2): ")),
    int(input("Enter Max HR (maximum heart rate achieved): ")),
    int(input("Enter Exercise Angina (1 = yes, 0 = no): ")),
    float(input("Enter Oldpeak (ST depression induced by exercise): ")),
    int(input("Enter ST Slope (0, 1, 2): "))
]
...
# Generate and print the report only if there is a risk of heart failure
if disease_type != "No Heart Failure":
    report = generate_prevention_report(risk, disease_type, input_data[0])
    print("\nGenerated Wellness Report:")
    print(report)
else:
    print("No report generated as there is no risk of heart failure.")
